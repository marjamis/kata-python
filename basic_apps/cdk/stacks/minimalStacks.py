...
class WaitConditionWithData(core.Stack):
  def __init__(self, scope: core.Construct, id: str, **kwargs) -> None:
    super().__init__(scope, id, **kwargs, description="A test on sending data to a WaitCondition. Once created to use: Send data output: cfn-signal -d <data> <wait_handle>")

    c = con.WaitConditionWithDataConstruct(self, "Blah", count=1, timeout="300")

    core.CfnOutput(self, "Outputs", value=c.getWaitCondition().get_att("Data").to_string(), description="Data for Cloudformation Handle using cfn-signal with data that is outputted into the Outputs")
